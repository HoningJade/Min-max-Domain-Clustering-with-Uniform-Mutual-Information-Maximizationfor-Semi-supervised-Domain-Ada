...
parser.add_argument('--steps', type=int, default=17001, metavar='N',
...
def train():
    ...
    for ste in range(all_step):
        ...
        if ste % args.update_pseudo==0  and ste>0 and p_update_count<args.num_shot:
            ...
            if p_update_count == 1:
                target_index = _means_target(source_all_feat, source_all_gt, source_all_pred, proto, target_feat, target_labels, p_update_count, args)
    ...
    #    lr = 0.01
        ...
        if not args.method == 'S+T':
            ...
            if args.method == 'ENT':
                ...
                loss_t = adaac(F1, f_Q, f_K, output, args.lamda, c_tau)
                loss_t.backward()
                optimizer_f.step()
                optimizer_g.step()
            else:
